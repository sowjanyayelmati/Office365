...
class HttpFulClient1 
{
  function createResource($accesstoken,$refreshtoken,$form_note_type,$assigned_to,$form_message_status)
  {
 
    $authorization_header = 'Bearer '.$accesstoken;
    $resource_uri='https://outlook.office365.com/api/v1.0/me/sendmail';
    $resource_body='{ "Message": { "Subject": "patient medical details", "Body": { "ContentType": "Text", "Content":"Type:'.$form_note_type.',To:'.$assigned_to.',Status:'.$form_message_status.'" }, "ToRecipients": [ { "EmailAddress": { "Address": "Admin@xtreamit.onmicrosoft.com" } } ], "Attachments": [ { "@odata.type": "#Microsoft.OutlookServices.FileAttachment", "Name": "menu.txt", "ContentBytes": "bWFjIGFuZCBjaGVlc2UgdG9kYXk=" } ] }, "SaveToSentItems": "True"}';
    $resource = \Httpful\Request::post($resource_uri)
      ->body($resource_body)
...
class HttpFulClient
{
  function getTokens1($refreshtoken)
  {
    # code...
    ...
    $token_body = 'grant_type=refresh_token&redirect_uri='.$redirect_uri.'&client_id=555629fa-2b38-4d70-b329-a8af9979c8e8&client_secret=4UxfCQMhN2X9uUSbIk%2Fm081zBF%2Bc0qB3zuaGoKhvqNw%3D&refresh_token='.$refreshtoken.'&resource=https://outlook.office365.com';
...
try {
    ...
    $httpfulClientResp = $httpfulClient->getTokens1($_SESSION["refresh_token"]);
    $_SESSION["access_token1"] = $httpfulClientResp->body->access_token;
    ...
    $accesstoken1=$_SESSION["access_token"];
   // echo $accesstoken1;
    //echo '<br><br>Access Token:--<br>';
...
switch($task) {
    case "add" :
    {
     ...
     $httpfulClient1 = new HttpFulClient1();
     //echo"note:";
       $note = $_POST['note'];
        $noteid = $_POST['noteid'];
        ...
        $assigned_to_list = explode(';', $_POST['assigned_to']);
         ...
         foreach($assigned_to_list as $assigned_to){
          if ($noteid && $assigned_to != '-patient-') {
            ...
            $noteid = '';
         //  $httpfulClientResp = $httpfulClient1->createResource($_SESSION['access_token'],$_SESSION['refresh_token']);
          }
          else {
            if($noteid && $assigned_to == '-patient-'){
           ...
           $httpfulClientResp = $httpfulClient1->createResource($accesstoken1,$_SESSION['refresh_token'],$form_note_type,$assigned_to,$form_message_status);
...
/*try {
    $httpfulClient1 = new HttpFulClient1();
    //$httpfulClientResp = $httpfulClient1->createResource($_SESSION['access_token'],$_SESSION['refresh_token']);
  //  echo "code: ".$httpfulClientResp->code;
   // echo "<br>";
   // echo "raw_headers: ".$httpfulClientResp->raw_headers;
   // echo "<br>";
} 
catch (Exception $e) 
{
